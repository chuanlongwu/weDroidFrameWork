...
public class WeDroidImageLoader {
	...
	private static String getMomeryCacheKeyForImageUri(String uri,Context context){//SKConstant.user_avatar_downLoad+userAccount
		List<String> cacheKeys = MemoryCacheUtil.findCacheKeysForImageUri(uri, WeDroidImageLoader.getImageLoader(context).getMemoryCache());
		return cacheKeys!=null&&!cacheKeys.isEmpty()?cacheKeys.get(0):null;
	}
	...
	public static void removeImageFromMomeryCache(String uri,Context context){
		...
		WeDroidImageLoader.getImageLoader(context).getMemoryCache().remove(key);
	}
	...
	public static File getFileForImageUri(String uri,Context context){//SKConstant.user_avatar_downLoad+userAccount
		...
		File file = DiscCacheUtil.findInCache(uri, WeDroidImageLoader.getImageLoader(context).getDiscCache());
