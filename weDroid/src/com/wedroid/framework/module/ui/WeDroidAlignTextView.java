...
public class WeDroidAlignTextView extends TextView {
	...
	public void initTextInfo() {
		...
		if (!TextUtils.isEmpty(text) && first) {
			...
			int totalHeight = (int) ((drawTotalLine+1)*textLineHeight+lineTextExtra*drawTotalLine + getPaddingBottom()+getPaddingTop()+layoutParams.bottomMargin+layoutParams.topMargin);
		...
		for (int i = 0; i < drawTotalLine; i++) {
			try {
				...
				if (endTextIndex > length) {
					endTextIndex = length;
					y += textLineHeight+lineTextExtra;
				} else {
					if(i==0){
						y = (int) textLineHeight;
					}else{
						y += textLineHeight+lineTextExtra;
					}
				}
...
//						c = (char) (c + 65248);
//					}
					canvas.drawText(String.valueOf(c), mLeft, y, mTextPaint);
	...
	int lineTextExtra;
	public void setLineTextExtra(int lineTextExtra){
		this.lineTextExtra = lineTextExtra;
	}
	 /** 
     * 判断是否为中文 
     * @return 
